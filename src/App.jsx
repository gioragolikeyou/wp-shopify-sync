...
function flattenWC(row) {
  ...
  // ── COPIA DESCRIPTION per custom_description (evita chiave duplicata nel mapping) ──
  if (flat["description"]) flat["_custom_description_copy"] = flat["description"];

  // ── YOAST: legge title e description da yoast_head_json ──
  if (row.yoast_head_json) {
    const yj = row.yoast_head_json;
    if (yj.title) flat["_yoast_title"] = yj.title;
    if (yj.og_description || yj.description) flat["_yoast_description"] = yj.og_description || yj.description;
  }

  // ── ATTRIBUTI WC: estrai ogni attributo come campo flat ──
  if (Array.isArray(row.attributes)) {
    row.attributes.forEach(attr => {
      const key = "attribute:" + (attr.slug || attr.name);
      const val = Array.isArray(attr.options) ? attr.options.join(", ") : (attr.option || "");
      if (val) flat[key] = val;
    });
  }
...
function buildPayload(entity, row, mapping, metaTypeMap) {
  ...
  Object.entries(mapping).forEach(([wpField,target])=>{
    ...
    if (target==="seo.title") {
      ...
      const plain = String(val || "").replace(/<br[^>]*>/gi,"\n").replace(/<\/p>/gi,"\n").replace(/<[^>]*>/g,"").replace(/&nbsp;/g," ").replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&#[0-9]+;/g,"").trim();
      const paragraphs = plain.split(/\n+/).filter(function(p){return p.trim();}).map(function(p){return {type:"paragraph",children:[{type:"text",value:p.trim()}]};});
  ...
  // ── DEDUP METAFIELD GLOBAL (seo.title/seo.description potrebbero essere pushati 2 volte) ──
  if (entity === "products" && Array.isArray(obj.metafields)) {
    const seen = new Set();
    obj.metafields = obj.metafields.filter(mf => {
