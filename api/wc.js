...
export default async function handler(req, res) {
  ...
  if (!wp_url || !wp_key || !wp_secret || !entity)
    return res.status(400).json({ error: "Parametri mancanti: wp_url, wp_key, wp_secret, entity" });
  if (!ALLOWED.includes(entity))
    return res.status(400).json({ error: `Entità non valida. Usa: ${ALLOWED.join(", ")}` });

  const base = wp_url.replace(/\/$/, "");
  const params = new URLSearchParams({
    consumer_key: wp_key, consumer_secret: wp_secret,
    per_page: String(Math.min(parseInt(per_page) || 100, 100)),
    page: String(page), orderby: "date", order: "desc",
  });
  ...
  try {
    ...
    return res.status(502).json({
      error: err.name === "TimeoutError" ? "Timeout — WP non risponde entro 20s" : "Errore di rete",
      detail: err.message,
    });
  }
}
